...
// chatHelpers.test.ts — chatHelpers.ts, api.ts's exported types, and
// types/chat.ts are unchanged by the port at the level this test exercises,
// so this carries over as-is.
...
describe('chatHelpers', () => {
  ...
  it('normalizeChatMath strips nested $ that break KaTeX', () => {
    expect(normalizeChatMath('$$ $F = ma$ $$')).toBe('$$F = ma$$')
    expect(normalizeChatMath('The law is \\( $F = ma$ \\).')).toBe('The law is $F = ma$.')
    expect(normalizeChatMath('Use $\\frac{1}{v} + \\frac{1}{u} = \\frac{1}{f}$')).toContain('\\frac{1}{f}')
    expect(normalizeChatMath('Use $\\frac{1}{v} + \\frac{1}{u} = \\frac{1}{f}$')).not.toContain('$$ $')
  })

  it('splitMarkdownSections keeps a single untitled block when there are no headings', () => {
    expect(splitMarkdownSections('Just a paragraph.')).toEqual([
      { heading: null, body: 'Just a paragraph.' },
    ])
  })

  it('splitMarkdownSections lists heading sections for copy', () => {
    const sections = splitMarkdownSections(
      'Intro line.\n\n## What it means\nInertia resists change.\n\n## Everyday example\nA book on a table stays put.'
    )
    expect(sections).toEqual([
      { heading: null, body: 'Intro line.' },
      { heading: 'What it means', body: 'Inertia resists change.' },
      { heading: 'Everyday example', body: 'A book on a table stays put.' },
    ])
  })

  it('aiMessageCopyText joins answer, formulas, and takeaway list', () => {
    const text = aiMessageCopyText(
      msg({
        id: 'a1',
        role: 'ai',
        content: 'Inertia resists change.',
        relatedFormulas: ['F = ma'],
        keyTakeaways: ['Stay at rest', 'Stay in motion'],
      })
    )
    expect(text).toContain('Inertia resists change.')
    expect(text).toContain('F = ma')
    expect(text).toContain('• Stay at rest')
    expect(text).toContain('• Stay in motion')
  })
