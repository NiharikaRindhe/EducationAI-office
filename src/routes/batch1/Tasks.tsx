...
export const Batch1Tasks: React.FC = () => {
  ...
  if (!assignments) {
    return <div className="flex justify-center py-16"><Loader2 className="animate-spin text-amber-400" /></div>;
  }
    ...
    <div className="flex flex-col gap-6 font-sans select-none anim-fade-up">
      ...
      <div className="bento-card border border-slate-100 bg-white p-5 flex flex-col gap-3">
        ...
        <div className="progress-bar bg-slate-100 h-3">
          <div className="progress-fill bg-indigo-600" style={{ width: `${(countCompleted / (assignments.length || 1)) * 100}%` }}></div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        {assignments.length > 0 ? (
          assignments.map((a) => (
            <div
              key={a.id}
              onClick={() => void handleCycle(a.id)}
              className={`bento-card bg-white border border-slate-100 p-5 ${STATUS_BORDER[a.status]} flex items-center justify-between gap-6 card-interactive cursor-pointer`}
            >
              <div className="flex items-center gap-4">
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center gap-2">
                    <span className="badge pill-amber text-[9px] font-black">{a.tasks.subject}</span>
                    <span className={`badge ${STATUS_PILL[a.status]} text-[9px] font-bold`}>{STATUS_LABEL[a.status]}</span>
                  </div>
                  <h4 className="font-display font-bold text-sm text-slate-800 mt-1">{a.tasks.title}</h4>
                </div>
              </div>
              ...
              <div className="flex items-center gap-5 select-none font-sans text-[11px] font-bold">
                {a.tasks.due_date && (
                  <span className="text-slate-400 flex items-center gap-1">
                    <Clock size={12} /> Due {a.tasks.due_date}
                  </span>
                )}
                <span className="badge pill-indigo text-[10px] font-black">+{a.tasks.xp_reward} XP</span>
              </div>
            </div>
          ))
        ) : (
