...
// PDF Simulator reader — re-enabled per user request (Aug 26 2026); see
// PDF_SIMULATOR_ENABLED in batch2/batch3 Layout.tsx (nav tab) and
// api/src/worker.ts (sim generation). See src/routes/shared/reader/ for the
// port itself.
const ReaderLibrary = lazy(() => import('./routes/shared/reader/routes/ReaderLibrary').then((m) => ({ default: m.ReaderLibrary })));
const ReaderSubjectChapters = lazy(() => import('./routes/shared/reader/routes/ReaderSubjectChapters').then((m) => ({ default: m.ReaderSubjectChapters })));
...
import { SuperAdminSimQa } from './routes/super-admin/SimQa';
        ...
        <AppProvider>
          ...
          <Routes>
            ...
            >
              ...
              {/* No Exams & Mocks component exists for Class 1-4 — genuinely
                  not built (age-appropriate exams weren't in scope), unlike
                  the batch2/batch3 redirects that were fixed alongside this. */}
            ...
            >
              ...
              <Route path="sim-qa" element={<SuperAdminSimQa />} />
              <Route path="sim-qa/:templateId" element={<SuperAdminSimQa />} />
