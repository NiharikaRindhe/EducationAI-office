...
describe('SimSpecSchema validation', () => {
  ...
  it('accepts ingest-time about and howItWorks on a template spec', () => {
    const parsed = SimSpecSchema.parse({
      version: '2.0',
      title: 'Textbook projectile',
      domain: 'physics',
      isSimulatable: true,
      templateId: 'projectile_2d',
      params: { v0: 20 },
      about: 'A ball thrown from the ground.',
      howItWorks: '- Horizontal speed stays the same.',
    })
    expect(parsed.about).toContain('ball')
    expect(parsed.howItWorks).toContain('Horizontal')
  })
